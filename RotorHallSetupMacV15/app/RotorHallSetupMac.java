...
public class RotorHallSetupMac extends JFrame {
    ...
    public RotorHallSetupMac() {
        ...
        // Formula - use BoxLayout for precise control: T = [tfA][H] [±] [tfB]
        g.gridy = 4;
        g.gridwidth = 2;
        g.gridx = 0;
        g.anchor = GridBagConstraints.CENTER;

        // Build a horizontal box so we can glue tfA and H tightly while
        // leaving small gaps around the sign and baseline value.
        JPanel formulaPanel = new JPanel();
        formulaPanel.setOpaque(false);
        formulaPanel.setLayout(new BoxLayout(formulaPanel, BoxLayout.X_AXIS));

    // small left spacer to nudge the whole formula to the right
    formulaPanel.add(Box.createHorizontalStrut(60));
    JLabel tLabel = lbl("T =", 32f, Font.BOLD);
    tLabel.setOpaque(true);
    formulaPanel.add(tLabel);
    formulaPanel.add(Box.createHorizontalStrut(10)); // small gap after 'T ='

        tfA = numField(6, 38, 200);
        formulaPanel.add(tfA);
        // glue H to tfA by using a very small strut
        formulaPanel.add(Box.createHorizontalStrut(2));

        JLabel hLabelSmall = lbl("H", 28f, Font.BOLD);
        hLabelSmall.setOpaque(true);
        // give the H a tiny left padding so it doesn't touch the text visually
        hLabelSmall.setBorder(new EmptyBorder(0, 4, 0, 10));
        formulaPanel.add(hLabelSmall);

        // small space before sign
        ...
        // small space after sign
        formulaPanel.add(Box.createHorizontalStrut(6));

        tfB = numField(6, 38, 200);
        formulaPanel.add(tfB);

        // add the assembled panel to the form, spanning across columns
        g.gridx = 0;
        g.gridwidth = 6;
        g.anchor = GridBagConstraints.BASELINE_LEADING;
        ...
        g.gridy = 6;
        g.gridwidth = 6;
        JLabel legendLabel = lbl(" T = Thickness, H = Hall Voltage", 30f, Font.PLAIN); // 이거 순서 바꿔서 수정함
        ...
        btnSet.addActionListener(e -> {
            try {
                ...
                GraphWindowFlat gw = new GraphWindowFlat(a, b, signMultiplier, cautionPct, dangerPct, radiusCm, port);
                gw.setVisible(true);
            } catch (Exception ex) {
    ...
    public static void main(String[] args) {
        // DLL을 Temp 폴더에 복사하고 로드 (Windows 전용)
        String os = System.getProperty("os.name", "").toLowerCase();
        String arch = System.getProperty("os.arch", "").toLowerCase();
        
        if (os.contains("win")) {
            try {
                // 1. 현재 JAR의 위치 찾기 (Windows 경로 문제 해결)
                ...
                // 3. app 폴더의 Windows/[arch]/jSerialComm.dll 찾기
                ...
                // 4. Temp 폴더에 DLL 복사 (jSerialComm가 찾는 위치)
...
class GraphWindowFlat extends JFrame {
    ...
    private final int signMultiplier; // +1 or -1, controls plus or minus for b
    ...
    // (formula fixed) thickness calculation is T = a * V - b
    ...
    GraphWindowFlat(double a, double b, int signMultiplier, double cautionPct, double dangerPct, double radiusCm, String port) {
        super("Graph");
        this.a = a;
        this.b = b;
        ...
        // formula is fixed: T = a * V - b (no selector)
        ...
        serial = new SerialReader(Vraw -> {
            ...
            // 두께 계산 수식: T = a * V - b (고정)
            double T = clamp(a * V + signMultiplier * b, 0, 5);

            vChart.push(dt, V);  // 첫 번째 그래프에 전압 표시
            tChart.push(dt, T);  // 두 번째 그래프에 두께 표시
        ...
        simulationTimer = new javax.swing.Timer(200, e -> {
            if (simulationMode) {
                ...
                // 시뮬레이션: T = a * V +/- b (controlled by signMultiplier)
                double T = clamp(a * V + signMultiplier * b, 0, 5);
        ...
        // (only one simulationTimer is defined — duplicates removed)
    }
    ...
    // no formula selector/label — formula fixed to T = a * V - b
